...
- 정본: 규칙은 [`AgentRules.md`](AgentRules.md)(요약은 `AGENTS.md` 15절), 결정 대기는 [`Tasks/decisions.md` '관리 체계'](Tasks/decisions.md) D-12~D-31.
...
3. **세션 시작 시 `AGENTS.md`를 세 에이전트가 모두 자동으로 읽게 하는 최소 변경**: 루트 `CLAUDE.md`에 `@AGENTS.md`(Claude Code 공식 권장), `.gemini/settings.json`에 `context.fileName` 배열(Gemini CLI 공식 예시), Codex·Antigravity·Cursor는 네이티브. 남은 것은 사용자 전역 파일 두 개의 "AGENTS.md 참고 하지 않는다" 줄 삭제(D-12·D-13). 근거: [research/agent-wiring.md](AgentCollaboration/research/agent-wiring.md).
...
- 장부: `NOW.md`, `Worklog/2026-09.md`, `Lessons/{build,editor,repo,anim}.md`(Tools/README.md 5절 6건 + 이번 세션 발견 이관). `Tasks/README.md`에 `담당` 필드·청구 규칙, `Tasks/decisions.md`에 '관리 체계' D-12~D-31.
...
- 하지 않은 것(사용자 결정 뒤): 전역 파일 수정, `.gemini/scripts`·`Tools/Animation`·Claude 홈 tools 이동, 추적 중 `.pyc` 해제, 기존 도구 개명, Claude 메모리 이관, `Docs/README.md` 전체 문서 인덱스, `tasks_recount.py` 확장, 검사 스크립트.

## 4. 다음 순서

1. 사용자: D-12·D-13(전역 파일 금지 줄) 결정 → 이것이 끝나야 Codex·Gemini가 `AGENTS.md`를 확실히 따른다.
...
4. 사용자: D-36(Blender 텔레메트리, 즉시)과 D-32(동시 운용·자원 임대) 결정. D-32가 (a)면 AgentRules 5절 활성 + `Tools/lock_resource.py` 제작(Backlog 11번).

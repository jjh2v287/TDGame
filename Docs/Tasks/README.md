...
- 담당: <agent>/<YYYY-MM-DD> (doing일 때 필수, done이면 비움; 식별자는 AGENTS.md 15절)
- 우선순위: 높음 | 중간 | 낮음
- 선행: P1-02
- 목표: 한 문장
- 완료 조건: 검증 가능한 조건 목록
...
1. 시작 순서는 `AGENTS.md` 15절(NOW → Worklog → 이 대장의 doing·blocked·decision → 착수 항목 본문)을 따른다. 7절 C++ 전용 로직을 지키고 아키텍처 문서(03)의 클래스·폴더 이름을 따른다.
2. 상태가 `todo`이고 선행 작업이 `done`인 항목 중 우선순위가 가장 높은 것을 고른다. 고르면 상태를 `doing`으로 바꾸고 `담당` 필드에 `<agent>/<날짜>`를 적은 뒤 `Docs/Worklog/`에 `시작` 항목을 쓴다(청구). 담당이 비어 있을 때만 청구하고 에이전트당 동시 doing은 1개다(OP-11).
3. 완료 조건을 전부 만족했을 때만 `done`으로 바꾼다. 검증 결과(테스트 출력, 로그 발췌, 체력·좌표 수치 등)를 기록 칸에 `YYYY-MM-DD 동사(<agent>): 내용` 형식으로 남긴다. 일부만 됐으면 `doing`을 유지하고 남은 조건을 적는다.
...
Phase별 완료 현황은 각 파일 맨 위 표를 `python Tools/tasks_recount.py`로 갱신한다(손으로 열을 바꾸지 않는다, OP-29).

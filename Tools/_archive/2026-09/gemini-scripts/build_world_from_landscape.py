# File: .gemini/scripts/build_world_from_landscape.py
import os
import sys
import json
import math
...
def build_from_landscape():
    ...
    # 1. Step 1: Ensure LV_DarkFantasy_OpenWorld is active
    ...
    # -------------------------------------------------------------
    # BATCH 1: Landscape Setup, Atmosphere, PostProcess & Perimeter
    # -------------------------------------------------------------
    print("[3/7] Configuring Real Landscape, Silver-Gray Lighting & Perimeter Ring...")
    batch1_script = '''
import json
...
def run():
    ...
    landscape = next((a for a in actors if "Landscape_UAID" in a["refPath"]), None)
    player_start = next((a for a in actors if "PlayerStart" in a["refPath"]), None)

    # 1. Configure Genuine Landscape (3km x 3km bounds, MI_GraySoil material)
    if landscape:
        execute_tool("editor_toolset.toolsets.actor.ActorTools.set_actor_transform", json.dumps({
            "actor": landscape,
            "xform": {
                "location": {"x": -151200.0, "y": -151200.0, "z": 0.0},
                "rotation": {"pitch": 0.0, "yaw": 0.0, "roll": 0.0},
                "scale": {"x": 300.0, "y": 300.0, "z": 100.0}
            },
            "worldspace": True
        }))
        execute_tool("editor_toolset.toolsets.object.ObjectTools.set_properties", json.dumps({
            "instance": landscape,
            "values": json.dumps({
                "landscapeMaterial": {"refPath": "/Game/DarkFantasyTopDown/Materials/Nature/Surfaces/MI_GraySoil.MI_GraySoil"}
            })
        }))

    # 2. PlayerStart at Village Plaza (0, 0, 100)
    if player_start:
        execute_tool("editor_toolset.toolsets.actor.ActorTools.set_actor_transform", json.dumps({
            "actor": player_start,
            "xform": {
                "location": {"x": 0.0, "y": -500.0, "z": 100.0},
    ...
    # 3. Directional Light: 3.2 lux silver-gray daylight
    if dir_light:
    ...
    # 4. Sky Light: 1.5 lux ashen ambient (bright open shadows)
    if sky_light:
    ...
    # 5. Fog: 0.005 density for open 3km horizon sightline
    if fog:
    ...
    # 6. PostProcessVolume for ashen ruin tone (0.65 saturation, 1.05 exposure)
    ppv = execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_class", json.dumps({
        "actor_type": {"refPath": "/Script/Engine.PostProcessVolume"},
        "name": "GlobalPostProcessVolume",
        "xform": {"location": {"x": 0.0, "y": 0.0, "z": 0.0}, "rotation": {"pitch": 0.0, "yaw": 0.0, "roll": 0.0}, "scale": {"x": 1.0, "y": 1.0, "z": 1.0}},
        "snap_to_ground": False
    }))["returnValue"]
    ppv_comps = execute_tool("editor_toolset.toolsets.actor.ActorTools.get_components", json.dumps({"actor": ppv}))["returnValue"]
    for c in ppv_comps:
    ...
    # 7. Perimeter Border Mountain Ring (28 Giant Cliffs at +-140,000 cm)
    ...
    for cfg in pcg_configs:
        ...
        if pcg_comp:
            props = execute_tool("editor_toolset.toolsets.object.ObjectTools.get_properties", json.dumps({
                "instance": pcg_comp, "properties": ["graphInstance"]
            }))["returnValue"]
            gi_ref = json.loads(props).get("graphInstance")
            if gi_ref:
                execute_tool("editor_toolset.toolsets.object.ObjectTools.set_properties", json.dumps({
                    "instance": gi_ref,
                    "values": json.dumps({"graph": {"refPath": cfg["graph"]}})
                }))
            execute_tool("editor_toolset.toolsets.object.ObjectTools.set_properties", json.dumps({
                "instance": pcg_comp,
                "values": json.dumps({"generationTrigger": "GenerateOnLoad", "bActivated": True})
            }))

    return {"status": "ok", "landscape": landscape is not None}
'''
    res1 = run_editor_script(batch1_script)
    print("  - Batch 1 Result:", res1[0]["text"] if res1 else "None")

    # -------------------------------------------------------------
    # BATCH 2: Undulating Terrain Hills & Geological Formations
    # -------------------------------------------------------------
    print("[4/7] Constructing Undulating Terrain Hills, Ridges & Formations...")
    batch2_script = '''
import json
...
def run():
    ...
    for gx in grid_coords:
        for gy in grid_coords:
            ...
            yaw_a = (idx * 47) % 360
            execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
                "asset_path": asset_a, "name": f"TerrainHill_A_{idx}",
                "xform": {"location": {"x": gx + ((idx*13)%3000 - 1500), "y": gy + ((idx*17)%3000 - 1500), "z": z_a}, "rotation": {"pitch": 0.0, "yaw": float(yaw_a), "roll": 0.0}, "scale": {"x": scale_a, "y": scale_a, "z": scale_a * 0.7}},
                "snap_to_ground": False
            }))
            spawned += 1
            ...
            yaw_b = (idx * 83) % 360
            execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
                "asset_path": asset_b, "name": f"TerrainHill_B_{idx}",
                "xform": {"location": {"x": gx + 4500.0, "y": gy - 4000.0, "z": z_b}, "rotation": {"pitch": 0.0, "yaw": float(yaw_b), "roll": 0.0}, "scale": {"x": scale_b, "y": scale_b, "z": scale_b * 0.8}},
                "snap_to_ground": False
            }))
            spawned += 1
            idx += 1
...
'''
    ...
    # -------------------------------------------------------------
    # BATCH 3: Complete Ashen Haven Village (Central Basin)
    # -------------------------------------------------------------
...
def run():
    ...
    def spawn_light(name, loc, intensity=35.0, color=(1.0, 0.5, 0.15), radius=1000.0):
        ...
        act = execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_class", json.dumps({
            "actor_type": {"refPath": "/Script/Engine.PointLight"}, "name": name,
        ...
        for c in comps:
            if "LightComponent" in c["refPath"]:
                execute_tool("editor_toolset.toolsets.object.ObjectTools.set_properties", json.dumps({
                    "instance": c,
                    "values": json.dumps({"intensity": intensity, "lightColor": {"r": color[0], "g": color[1], "b": color[2], "a": 1.0}, "attenuationRadius": radius})
                }))
        spawned += 1
...
'''
    ...
    # -------------------------------------------------------------
    # BATCH 4: Dense Forest Groves across the 3km World (36 Groves)
    # -------------------------------------------------------------
    ...
    for cx in grove_coords:
        for cy in grove_coords:
            ...
            for t_i, (ox, oy) in enumerate(cluster_offsets):
                ...
                execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
                    "asset_path": t_asset, "name": f"Grove_{g_idx}_Tree_{t_i}",
                    "xform": {"location": {"x": tx, "y": ty, "z": 0.0}, "rotation": {"pitch": 0.0, "yaw": t_yaw, "roll": 0.0}, "scale": {"x": t_scale, "y": t_scale, "z": t_scale}},
                    "snap_to_ground": False
                }))
                spawned += 1

            execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
                "asset_path": bushes, "name": f"Grove_{g_idx}_Bush_1",
                "xform": {"location": {"x": cx + 500.0, "y": cy - 400.0, "z": 0.0}, "rotation": {"pitch": 0.0, "yaw": float((g_idx * 53) % 360), "roll": 0.0}, "scale": {"x": 1.6, "y": 1.6, "z": 1.6}},
                "snap_to_ground": False
            }))
            execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
                "asset_path": flora[g_idx % len(flora)], "name": f"Grove_{g_idx}_Flora",
                "xform": {"location": {"x": cx - 600.0, "y": cy + 300.0, "z": 0.0}, "rotation": {"pitch": 0.0, "yaw": float((g_idx * 29) % 360), "roll": 0.0}, "scale": {"x": 1.8, "y": 1.8, "z": 1.8}},
                "snap_to_ground": False
            }))
            spawned += 2
            g_idx += 1
...
'''
    ...
    # -------------------------------------------------------------
    # BATCH 5: Cobblestone Road Network & 4 Regional Landmarks
    # -------------------------------------------------------------
...
def run():
    ...
    for idx, d in enumerate(road_distances):
        ...
        # South Road
        execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
            "asset_path": tile_asset, "name": f"RoadTile_S_{idx}",
            "xform": {"location": {"x": 0.0, "y": -d, "z": 5.0}, "rotation": {"pitch": 0.0, "yaw": 0.0, "roll": 0.0}, "scale": {"x": 2.8, "y": 2.8, "z": 1.0}},
            "snap_to_ground": False
        }))
        execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
            "asset_path": lamp_asset, "name": f"RoadLamp_S_{idx}",
            "xform": {"location": {"x": -400.0, "y": -d, "z": 0.0}, "rotation": {"pitch": 0.0, "yaw": -90.0, "roll": 0.0}, "scale": {"x": 1.6, "y": 1.6, "z": 1.6}},
            "snap_to_ground": False
        }))
        ...
        # West Road
        execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
            "asset_path": tile_asset, "name": f"RoadTile_W_{idx}",
            "xform": {"location": {"x": -d, "y": 0.0, "z": 5.0}, "rotation": {"pitch": 0.0, "yaw": 90.0, "roll": 0.0}, "scale": {"x": 2.8, "y": 2.8, "z": 1.0}},
            "snap_to_ground": False
        }))
        execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
            "asset_path": lamp_asset, "name": f"RoadLamp_W_{idx}",
    ...
    ny = 85000.0
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": "/Game/DarkFantasyTopDown/StaticMeshes/Nature/Cliff/SM_Cliffs1", "name": "POI_Citadel_Wall",
        "xform": {"location": {"x": 0.0, "y": ny + 4000.0, "z": 0.0}, "rotation": {"pitch": 0.0, "yaw": 0.0, "roll": 0.0}, "scale": {"x": 4.5, "y": 4.5, "z": 4.0}},
        "snap_to_ground": False
    }))
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": "/Game/DarkFantasyTopDown/StaticMeshes/WoodenParts/SM_WoodenConstruction1", "name": "POI_Citadel_Tower_L",
        "xform": {"location": {"x": -1800.0, "y": ny, "z": 0.0}, "rotation": {"pitch": 0.0, "yaw": 25.0, "roll": 0.0}, "scale": {"x": 2.2, "y": 2.2, "z": 2.5}},
        "snap_to_ground": False
    }))
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": "/Game/DarkFantasyTopDown/StaticMeshes/WoodenParts/SM_WoodenConstruction1", "name": "POI_Citadel_Tower_R",
        "xform": {"location": {"x": 1800.0, "y": ny, "z": 0.0}, "rotation": {"pitch": 0.0, "yaw": -25.0, "roll": 0.0}, "scale": {"x": 2.2, "y": 2.2, "z": 2.5}},
        "snap_to_ground": False
    }))
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": "/Game/DarkFantasyTopDown/StaticMeshes/Lanterns/SM_Bowl", "name": "POI_Citadel_Brazier",
    ...
    sy = -85000.0
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": "/Game/DarkFantasyTopDown/StaticMeshes/Rocks2/SM_Rock2", "name": "POI_Necro_Megalith1",
        "xform": {"location": {"x": -1500.0, "y": sy, "z": 0.0}, "rotation": {"pitch": 0.0, "yaw": 45.0, "roll": 0.0}, "scale": {"x": 3.0, "y": 3.0, "z": 4.0}},
        "snap_to_ground": False
    }))
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": "/Game/DarkFantasyTopDown/StaticMeshes/Rocks2/SM_Rock2", "name": "POI_Necro_Megalith2",
        "xform": {"location": {"x": 1500.0, "y": sy, "z": 0.0}, "rotation": {"pitch": 0.0, "yaw": -45.0, "roll": 0.0}, "scale": {"x": 3.0, "y": 3.0, "z": 4.0}},
        "snap_to_ground": False
    }))
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": "/Game/DarkFantasyTopDown/StaticMeshes/WoodenParts/SM_Cart", "name": "POI_Necro_WreckedCart",
    ...
    ex = 85000.0
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": "/Game/DarkFantasyTopDown/StaticMeshes/Nature/Cliff/SM_Cliff3", "name": "POI_East_Gate_L",
        "xform": {"location": {"x": ex, "y": -2200.0, "z": 0.0}, "rotation": {"pitch": 0.0, "yaw": -20.0, "roll": 0.0}, "scale": {"x": 3.2, "y": 3.2, "z": 4.0}},
        "snap_to_ground": False
    }))
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": "/Game/DarkFantasyTopDown/StaticMeshes/Nature/Cliff/SM_Cliff3", "name": "POI_East_Gate_R",
        "xform": {"location": {"x": ex, "y": 2200.0, "z": 0.0}, "rotation": {"pitch": 0.0, "yaw": 20.0, "roll": 0.0}, "scale": {"x": 3.2, "y": 3.2, "z": 4.0}},
        "snap_to_ground": False
    }))
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": tile_asset, "name": "POI_East_Altar",
    ...
    wx = -85000.0
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": "/Game/DarkFantasyTopDown/StaticMeshes/Nature/Dreadplants/SM_Mycelium1", "name": "POI_West_Mycelium1",
        "xform": {"location": {"x": wx, "y": -1200.0, "z": 0.0}, "rotation": {"pitch": 0.0, "yaw": 30.0, "roll": 0.0}, "scale": {"x": 3.0, "y": 3.0, "z": 3.0}},
        "snap_to_ground": False
    }))
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": "/Game/DarkFantasyTopDown/StaticMeshes/Nature/Dreadplants/SM_Mycelium1", "name": "POI_West_Mycelium2",
        "xform": {"location": {"x": wx + 800.0, "y": 1400.0, "z": 0.0}, "rotation": {"pitch": 0.0, "yaw": -60.0, "roll": 0.0}, "scale": {"x": 2.5, "y": 2.5, "z": 2.5}},
        "snap_to_ground": False
    }))
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": "/Game/DarkFantasyTopDown/StaticMeshes/Rocks2/SM_Rock3", "name": "POI_West_Boulder",
        "xform": {"location": {"x": wx - 500.0, "y": 0.0, "z": 0.0}, "rotation": {"pitch": 0.0, "yaw": 90.0, "roll": 0.0}, "scale": {"x": 3.5, "y": 3.5, "z": 3.5}},
        "snap_to_ground": False
    }))
    spawned += 3

    # Dungeon Atlas Slot 1 at safe 5km distance (X: 500,000)
    dungeon_x = 500000.0
    for rx in range(-2, 3):
        for ry in range(-2, 3):
            execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
                "asset_path": tile_asset, "name": f"DungeonAtlas_Floor_{rx}_{ry}",
    ...
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": "/Game/DarkFantasyTopDown/StaticMeshes/Nature/Cliff/SM_Cliff2", "name": "DungeonAtlas_WallN",
        "xform": {"location": {"x": dungeon_x, "y": 700.0, "z": -20.0}, "rotation": {"pitch": 0.0, "yaw": 0.0, "roll": 0.0}, "scale": {"x": 1.8, "y": 1.8, "z": 2.0}},
        "snap_to_ground": False
    }))
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": "/Game/DarkFantasyTopDown/StaticMeshes/Nature/Cliff/SM_Cliff2", "name": "DungeonAtlas_WallS",
        "xform": {"location": {"x": dungeon_x, "y": -700.0, "z": -20.0}, "rotation": {"pitch": 0.0, "yaw": 180.0, "roll": 0.0}, "scale": {"x": 1.8, "y": 1.8, "z": 2.0}},
        "snap_to_ground": False
    }))
    execute_tool("editor_toolset.toolsets.scene.SceneTools.add_to_scene_from_asset", json.dumps({
        "asset_path": "/Game/DarkFantasyTopDown/StaticMeshes/Lanterns/SM_Bowl", "name": "DungeonAtlas_Portal",
...
'''
    ...
    # -------------------------------------------------------------
    # BATCH 6: Save and Synchronize to L_TDWorld_Main
    # -------------------------------------------------------------
    print("Saving assets and synchronizing to /Game/World/Maps/L_TDWorld_Main...")
    client.call_tool("editor_toolset.toolsets.asset.AssetTools", "save_assets", {"asset_paths": []})
    client.call_tool("editor_toolset.toolsets.asset.AssetTools", "delete", {"path": "/Game/World/Maps/L_TDWorld_Main"})
    client.call_tool("editor_toolset.toolsets.asset.AssetTools", "duplicate", {
        "path": "/Game/Level/LV_DarkFantasy_OpenWorld",
        "new_path": "/Game/World/Maps/L_TDWorld_Main"
    })
    client.call_tool("editor_toolset.toolsets.asset.AssetTools", "save_assets", {"asset_paths": []})
    print("[SUCCESS] 3km Open World with Real Landscape Successfully Built & Synchronized!")

if __name__ == "__main__":
    build_from_landscape()
